...
- 「コミット」とは「毎月 $100 をデポジット式に入金する約束」のこと。
  払った $100 は消えず**アカウント残高に積み上がり**、他の DFS API でも使える。
- 契約すると以下が解禁:
...
**🚫 2026-06-15: このプロジェクトでは本契約しない方針に決定**

理由 (実証検証で判明):
- DFS Labs の日本語 KW インデックスは**ロングテール KW のカバレッジが極端に薄い**
- 実検証: 1語 (商標/カテゴリ) や 2語の自然な組み合わせは取れるが、
  **「○○ おすすめ」 「○○ 安い」 「○○ レビュー」 等の主婦向け CVKW は 0% カバー**
- お宝発掘の核心領域 (CVKW) で KD が手に入らない = 月 ¥17,800 の投資対効果が低い

撤退スケジュール:
- 2026-06-14 〜 6/28: 14日間トライアル維持 (一部 KW で KD は取れるので運用検証は可能)
- 2026-06-28: **本契約しない** (Activate ボタンを押さない → 自動失効)
- 2026-07-14 まで: **初回入金 $50 のリファンド申請** (DFS Terms の 30-day no-question 適用)

KD は「取れたらラッキー」 の補助指標として運用継続 (Stage 2.5 の bulkKeywordDifficulty 呼び出しは
失敗時フォールバックで継続動作する)。お宝スコア (treasureScore.ts) は KD 抜き 70点満点でも
60点超え (お宝判定) が成立する設計。
