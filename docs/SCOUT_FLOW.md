...
## 1.5 UIバッジ (5個) と内部Stage (6段) の対応 ★よく混乱するポイント

KWスカウト画面の上部に横並びで出る5個のバッジは、内部6段パイプラインの
**一部だけ**を抜き出して表示しています。**Stage 2 と Stage 3.5 は「裏方（DFSで数値を取るだけ）」なのでバッジに出ません。**
このため「バッジの番号」と「内部Stageの番号」がズレて見えるのが混乱の原因。

### 対応表

| UIバッジ | 内部Stage | 担当 | 数字の意味 |
|---|---|---|---|
| 🌱 (双葉) | **Stage 0** | DFS#1 | 実検索シードの件数（Geminiへの入力ネタ元。絞り込み対象ではない） |
| 🤖 (ロボ) | **Stage 1** | Gemini#1 | 生成したKW**全件**（絞り込み前） |
| — | 〔Stage 2〕 | DFS#2 | **非表示**。SV/CPC/競合度を取得するだけ（捨てない） |
| 🔍 (虫眼鏡) | **Stage 3** | Gemini#2 | 数値＋重複排除で**1次絞り込みを通過**した件数 |
| — | 〔Stage 3.5〕 | DFS#3 | **非表示**。Google公式intentラベルを取得するだけ |
| 🌐 (地球儀) | **Stage 4** | DFS#4 | SERP取得＋お宝スコア計算を**やった**件数（= Stage3通過数と同じ） |
| 🎯 (的) | **Stage 5** | Gemini#3 | **最終採用**した件数（decision="adopt"） |

### バッジを縦に並べるとファネルが見える

```
🌱 1     ← Stage0 シード（材料。絞り込みではない）
🤖 472   ← Stage1 生成（全件）
  〔Stage2 DFS数値取得＝裏方・非表示〕
🔍 10    ← Stage3 絞り込み通過      ◀── 大きく絞られるのはここ（472→10）
  〔Stage3.5 intent取得＝裏方・非表示〕
🌐 10    ← Stage4 SERP+スコア計算（10件のまま。絞り込みではない）
🎯 2     ← Stage5 最終採用          ◀── 2か所目の絞り込み（10→2）
```

### 覚えておくべき3点

1. **件数が実際に絞られるのは2か所だけ** → `🤖472 → 🔍10`（数値+AI絞り込み）と `🌐10 → 🎯2`（AI最終判定）。
2. **🌐(Stage4) は絞り込みではない** → 🔍の10件にSERPとお宝スコアを「付けるだけ」なので数は変わらないのが正常。
3. **安いフィルタが先・高いフィルタが後** → DFS SERP APIは高い(¥17)ので、Stage3で10件に絞ってからSERPを叩く設計。472件全部にSERPを打つとコスト爆発する。
