...
// note公式ヘルプセンター (help-note.com) のガイド3記事を元に作成:
//   1. エディタ（記事編集画面）でできること
//   2. タイトルと見出し画像のコツ
//   3. もっとも大事なこと
// 詳細は project_note_automation メモリ参照。
export const NOTE_OFFICIAL_TIPS = `
【note公式ガイド準拠ルール（タイトル・見出し画像・哲学）】

# タイトル
1. **24文字以内を厳守** (note検索結果で25文字までしか表示されない)
2. **キャッチコピー要素** を必ず加えて高揚感を出す（読者の好奇心・感情を刺激）
3. **メインKWを必ず含める** (検索流入の入口になる)
4. **カナと漢字のバランス** に気をつける（カタカナだけ／漢字の塊は避ける）
5. **句読点を付けない** — タイトル末尾に「。」は付けない。「！」「？」は感情を強く伝えたい時のみ可
6. **誤字脱字ゼロ** (信頼性が落ちる)

# 見出し画像（image_prompt_subject）
1. **記事内容との関連性** が最重要（装飾だけの画像にしない）
2. **推奨サイズは 1280 × 670 px** — image_prompt_subject は横長画像が想定されるテーマで作る
3. **大きなシルエット中心** で空白（余白）を意識した構図
4. テキスト入り画像なら短く、画像自体で内容を視覚的に伝える

# 本文哲学（note公式の最重要メッセージ — もっとも大事なこと）
- **創作を楽しみ続けること** ＞ PV ＞ フォロワー数 ＞ 収益
- **発表し続けること** が一番大事。名文を仕上げようとして手が止まるくらいなら、短文でも気軽に投稿する
- 創作活動は筋トレと同じ。ちょっとずつ日常生活の一部にして「クセ」をつける
- → AI生成記事も「肩肘張らない・読者と距離が近い」温度感を維持する

# noteエディタの正しい使い方（コピペを想定した最適化）
1. **タイトルは独立フィールド** — note は「タイトル」「本文」が別入力欄。Markdown の # は本文には含めない (best_title フィールドを別途コピペするだけ)
2. **目次は H2/H3 から自動生成** — note は ## (大見出し) と ### (小見出し) を拾って目次を自動で出す
   - H2 のタイトルは「単独で読んで意味が通る名詞句」にする (目次の項目名になるため)
   - H2 と H3 の階層をきれいに保ち、フラットすぎず深すぎず
3. **冒頭150字が自動抜粋** — 記事一覧/SNS シェアカード/note 検索結果でこの150字が表示される。「主KW＋ベネフィット＋共感」を凝縮
4. **引用ブロック ">"** は3用途を使い分け:
   - 他者の発言・読者の心の声
   - ポイント/結論ボックス
   - 警告・注意喚起
5. **区切り線 "---"** で章の空気を切り替える（章間ではなく章内の山場前後にも有効）
6. **箇条書き** は意味付き絵文字 (✅❌💡📝⚠️✨) を使い分ける
7. **太字 \`**\`** は要所のみ。過剰使用は強調が薄れる
8. **画像埋込・サービス埋込** は AI 生成時には扱わない (Twitter/YouTube 等の埋込は手動で追加してもらう想定)
`.trim();
...
【note エディタへのコピペを前提とした出力構造】
- **タイトルは best_title フィールドのみに入れる**。body_markdown には H1 (#) を含めない (タイトルは note の専用入力欄にコピペするため、body 側に書くと重複してしまう)
- **大見出しは H2 (##)** で書く → note エディタで「大見出し」になり、目次に自動掲載される
- **小見出しは H3 (###)** で書く → note エディタで「小見出し」になり、目次にサブ項目として並ぶ
- 目次に並ぶことを意識し、H2 / H3 のタイトルは **単独で読んでも意味が通る** 名詞句にする（「詳しく見てみよう」NG、「保育園欠席連絡が面倒な3つの理由」OK）
- **冒頭の最初の段落 (150字以内)** が note の自動抜粋として記事一覧/SNS シェア時に表示されるので、ここに「主KW＋ベネフィット＋共感」を凝縮

【構成テンプレート（必ずこの順・各H2の頭に必ず絵文字・質問形またはストレート形）】

（**注意: body_markdown は下記の「> ✨ この記事でわかること」から始める。H1 タイトル行は含めない**）

> **✨ この記事でわかること**
> - ✅ 3〜5個の箇条書き（TL;DR・LLMが拾いやすい要約・**メインKWを1つ含める**）

（↓この最初の本文段落が **冒頭150字最適化エリア**。主KW＋ベネフィット＋共感を凝縮。note の自動抜粋・SNS シェアカードに使われる）
...
- **body_markdown には H1 (#) を含めない** — タイトルは best_title フィールドのみ。note のタイトル欄に別途コピペするため
- 見出しは ## (大見出し/目次に並ぶ) と ### (小見出し/目次にサブ項目) のみ使う
- H2 / H3 のタイトルは目次の項目名になるので、**単独で読んで意味が通る** 形にする (「詳しく見てみよう」NG、具体的な名詞句 OK)
...
出力は厳密に以下のJSONだけ（前後の説明文は禁止）:
{
  ...
  "body_markdown": "記事本文（Markdown・H1(#)は絶対に含めない・タイトルはbest_titleフィールドのみ・必ず構成テンプレート/LLMO/SEO/装飾ルールに全て従う・FIXED_AUTHOR_BIO_PLACEHOLDERはそのまま残しておくこと、後処理で実際の固定文に置換します）",
