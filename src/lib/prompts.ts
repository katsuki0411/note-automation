...
# noteエディタの正しい使い方（Markdown構造）
1. **H2 / H3 で適切に構造化** — noteは自動で目次を生成する
2. **引用ブロック ">"** は3用途を使い分け:
   - 他者の発言・読者の心の声
   - ポイント/結論ボックス
   - 警告・注意喚起
3. **区切り線 "---"** で章の空気を切り替える（章間ではなく章内の山場前後にも有効）
4. **箇条書き** は意味付き絵文字 (✅❌💡📝⚠️✨) を使い分ける
5. **太字 \`**\`** は要所のみ。過剰使用は強調が薄れる
`.trim();
...
# {タイトル（メインKWを先頭〜中盤に含む・**24文字以内厳守**・末尾に「。」付けない）}
