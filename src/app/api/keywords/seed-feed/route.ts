...
export async function POST(req: NextRequest) {
  try {
    ...
    if (isProviderAvailable("brave") || isProviderAvailable("google")) {
      ...
      const allDomains = await getAllowedPlatformDomains();
    ...
    // 3. KW を辞書に登録（あれば再利用）
    let kwRecord = await findKeyword(kw, themeId);
    if (!kwRecord && addToDictionary) {
      ...
      kwRecord = await createKeyword(kwInput);
    }
    ...
    // 5. Feedに追加
    const result = await appendIdeas([feedIdea]);
